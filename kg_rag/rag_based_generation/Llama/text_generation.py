...
def main():
    ...
    if not INTERACTIVE:
        ...
        # context = retrieve_context(question, vectorstore, embedding_function_for_context_retrieval, node_context_df, CONTEXT_VOLUME, QUESTION_VS_CONTEXT_SIMILARITY_PERCENTILE_THRESHOLD, QUESTION_VS_CONTEXT_MINIMUM_SIMILARITY, EDGE_EVIDENCE, llm_type='llama', llm=llm)
        print("Here is the KG-RAG based answer using Llama:")
        print("")
        template = get_prompt(INSTRUCTION, SYSTEM_PROMPT)
        prompt = PromptTemplate(template=template, input_variables=["context", "question"])
        # llm = llama_model(MODEL_NAME, BRANCH_NAME, CACHE_DIR, stream=False, method=METHOD) 
