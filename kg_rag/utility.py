...
# login(token=os.environ.get('HF_GATED_REPOS_TOKEN'))
...
def get_gpt():
    chat_model_id = config_data['GPT_MODEL']
    chat_deployment_id = chat_model_id if openai.api_type == 'azure' else None
    return chat_model_id, chat_deployment_id

def disease_entity_extractor(text):
    chat_model_id, chat_deployment_id = get_gpt()
...
def disease_entity_extractor_v2(text, llm_type='gpt', llm=None):
    if llm_type == 'gpt':
        chat_model_id, chat_deployment_id = get_gpt()
        ...
        template = get_prompt(instruction, system_prompts["DISEASE_ENTITY_EXTRACTION"])
...
def retrieve_context(question, vectorstore, embedding_function, node_context_df, context_volume, context_sim_threshold, context_sim_min_threshold, edge_evidence, llm_type='gpt', llm=None, api=True):
    entities = disease_entity_extractor_v2(question, llm_type=llm_type, llm=llm)
    node_hits = []
    if entities:
...
def interactive(question, vectorstore, node_context_df, embedding_function_for_context_retrieval, llm_type, edge_evidence, system_prompt, api=True, llama_method="method-1"):
    ...
    print("Processing ...")
    entities = disease_entity_extractor_v2(question, llm_type=llm_type)
